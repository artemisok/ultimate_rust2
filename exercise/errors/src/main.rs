...
fn play_time(dolphin: &Dolphin) -> Result<Vec<String>> {
    ...
    let response = r#try!(dolphin.say_your_name());
    println!("fitifiti {}",response);
    responses.push( response);
    // 2c. Do the same thing as #2b for the .flip() method
    let flips = r#try!(dolphin.flip());

    responses.push( flips ) ;

    // 2d. Do the same thing as #2b for the .shake_hands() method
    let hands = r#try!(dolphin.shake_hands());

    responses.push( hands) ;
        ...
        match play_time(dolphin) {
            Ok(responses) => {
                ...
                for response in responses {
                    println!(" I am printed here {}", response);
                }
            }
